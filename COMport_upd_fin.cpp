...
bool setParityControl(uint8_t*& data, uint32_t dataSize) {
	uint64_t bitArraySize = 8 * dataSize;
	...
	for (uint64_t i = 0; i < bitArraySize; i++)
	{
		if (bitArray[i]) {
			bitCounter++;
		}
...
bool checkParityControl(uint8_t* data, uint32_t dataSize) {
	//uint8_t check = data[dataSize - 1];
	uint64_t bitArraySize = 8 * (dataSize - 1);
	uint8_t* bitArray = (uint8_t*)malloc(bitArraySize);
	uint8_t binaryNumber[] = { 0,0,0,0,0,0,0,0 };
	uint8_t decimalNumber;
	uint8_t j = 0;
...
bool SerialPort::comWritePacket(const uint8_t* data, uint32_t dataSize) {
	...
	uint64_t bitArraySize = 8 * (dataSize + HEADER_SIZE - 1);
	uint8_t* bitArray = (uint8_t*)malloc(bitArraySize);
	if (!bitArray) {
		free(packet);
		return false;
	...
	for (uint64_t i = 0; i < bitArraySize - 4; i++)
	{
		if (bitArray[i] && bitArray[i + 1] && bitArray[i + 2] && bitArray[i + 3] && bitArray[i + 4]) {
			...
			for (uint64_t j = bitArraySize - 1; j >= i + 5; j--)
			{
...
bool SerialPort::comReadPacket(uint8_t* data, uint32_t dataSize) {
	...
	bool initialFlagRecieved = false;
	...
	if (secondFlagRecieved) {
		initialFlagRecieved = true;
		packetSize++;
	...
	while (1) {
		...
		if (overlapped.InternalHigh != 1) {
			this->comDisconnect();
			if(!this->comInit(portName, baudRate, true)) {
				free(byteRecieved);
				free(packet);
				CloseHandle(readingPacketRecieved);
				secondFlagRecieved = false;
		...
		if (byteRecieved[0] == 0b01111110 && initialFlagRecieved == true) {
			secondFlagRecieved = true;
			break;
		}
		else if (byteRecieved[0] == 0b01111110 && initialFlagRecieved == false) {
			initialFlagRecieved = true;
		}

		if (initialFlagRecieved) {
			packetSize++;
			packet = (uint8_t*)realloc(packet, packetSize);
			if (!packet) {
				free(byteRecieved);
	...
	uint64_t bitArraySize = 8 * recievedSize;
	uint8_t decimalNumber;
	uint8_t binaryNumber[] = { 0,0,0,0,0,0,0,0 };
	uint64_t bitArrayPointer = 0;
	uint8_t* bitArray = (uint8_t*)malloc(bitArraySize);
	...
	for (uint64_t i = 0; i < bitArraySize - 4; i++)
	{
		if (bitArray[i] && bitArray[i + 1] && bitArray[i + 2] && bitArray[i + 3] && bitArray[i + 4]) {
			...
			for (uint64_t j = i + 5; j < bitArraySize - 2; j++)
			{
				bitArray[j] = bitArray[j + 1];
			}
	...
	if ((bitArraySize / 8) < dataSize) {
		dataSize = bitArraySize / 8 - 2;
	}

	for (uint32_t i = 16; i < (dataSize + 2)* 8; i++)
	{
		decimalNumber = 0;
		for (uint8_t j = 0; j < 8; j++, i++)
		{
