...
bool SerialPort::comWritePacket(const uint8_t* data, uint32_t dataSize) {
	...
	if (WriteFile(comPort, packet, staffedDataSize + HEADER_SIZE, &feedback, &overlapped)) {
...
bool SerialPort::comReadPacket(uint8_t* data, uint32_t dataSize) {
	...
	uint8_t* packet = new uint8_t[HEADER_SIZE];
	...
	//while (attempts) {
		...
		//if (!ReadFile(comPort, (LPVOID)packet, dataSize * 2 + 11, &feedback, &overlapped)) {
			ReadFile(comPort, (LPVOID)packet, 11, &feedback, &overlapped);
			...
			ReadFile(comPort, (LPVOID)packet, packetSize, &feedback, &overlapped);
			while (overlapped.InternalHigh < packetSize);
